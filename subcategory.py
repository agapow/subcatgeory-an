...
class SubCategory(URLWrapper):
    def __init__(self, name, parent, settings):
        ...
        if not hasattr (parent, 'subcats'):
            parent.subcats = []
        parent.subcats.append (self)
...
def create_subcategories(generator):
    # iterate ovcer all articles to collect all subcats?
    ...
    generator.context['subcategories'] = generator.subcategories

def generate_subcategories(generator, writer):
    # write the subcategory file
    generator.context['subcategories'] = generator.subcategories # PMA
